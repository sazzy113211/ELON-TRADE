...
""")
    ...
    def print_usage_tips(self):
        """Print usage tips for interacting with the deployed contract using Aptos CLI"""
        print("\n=== Usage Tips (Aptos CLI) ===")
        print(f"1. Request Access (requires transaction):")
        print(f"   - Call the `request_access` function using your account.")
        print(f"   - Example:")
        print(f"""
        aptos move run \\
            --function-id {self.account.address()}::{MODULE_NAME}::request_access \\
            --private-key <your_private_key> \\
            --url {NODE_URL}
        """)
        
        print(f"\n2. Store Encrypted Keys (requires transaction):")
        print(f"   - Call the `store_keys` function to store encrypted keys for a user.")
        print(f"   - Example:")
        print(f"""
        aptos move run \\
            --function-id {self.account.address()}::{MODULE_NAME}::store_keys \\
            --args address:<user_address> vector<u8>:<hex_encoded_encrypted_key_data> \\
            --private-key <your_private_key> \\
            --url {NODE_URL}
        """)
        print(f"   - Note: Replace `<user_address>` with the target user's address and `<hex_encoded_encrypted_key_data>` with the hex-encoded encrypted key data.")
        
        print(f"\n3. Retrieve Encrypted Keys (view function):")
        print(f"   - Call the `get_encrypted_keys` function to retrieve encrypted keys for a user.")
        print(f"   - Example:")
        print(f"""
        aptos move view \\
            --function-id {self.account.address()}::{MODULE_NAME}::get_encrypted_keys \\
            --args address:<requester_address> address:<user_address> \\
            --url {NODE_URL}
        """)
        print(f"   - Note: Replace `<requester_address>` with the address requesting access and `<user_address>` with the target user's address.")
        
        print("\nFor more details, refer to the Aptos CLI documentation: https://aptos.dev/cli")
